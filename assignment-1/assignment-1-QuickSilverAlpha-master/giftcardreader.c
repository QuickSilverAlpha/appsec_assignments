...
void animate(char *msg, unsigned char *program)
{
    ...
    while (pc < program + 256)
    {
        ...
        case 0x00:
            break;
        case 0x01:
            // Fix for crash2.gft, addings bound check before updating the array
            // regs[arg1] = *mptr;
            if (arg1 < 0 || arg1 > 15)
            {
                break;
        ...
        case 0x02:
            *mptr = regs[arg1];
            break;
        case 0x03:
            // Fix for fuzzer2.gft, mptr should always be bounded by message length which is 32 bytes.
            if (mptr < msg || mptr > msg + 31)
            {
                break;
            }
            mptr += (char)arg1;
            break;
        case 0x04:
            // Fix for fuzzer2.gft, added out of bound rights on the regs array
            if (arg2 < 0 || arg2 > 15)
            {
                break;
            }
        ...
        case 0x08:
            goto done;
        case 0x09:
            // Changed char to unsigned char to fix hang.gft
...
struct this_gift_card *gift_card_reader(FILE *input_fd)
{
    ...
    while (!feof(input_fd))
    {
        ...
        {
            printf("Invalid gift card! Specified corrupted length!\n");
            exit(1);
        }
