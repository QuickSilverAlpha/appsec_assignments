...
def buy_card_view(request, prod_num=0):
    if request.method == 'GET':
        ...
        if director is not None:
            ...
            '''RA: Escaping HTML via HTML encoding and not trusting the user as well as the framework (default django 
            escape, better code practise!)'''
...
def gift_card_view(request, prod_num=0):
    ...
    if request.method == "GET" and 'username' not in request.GET:
        ...
        # RA: Commented this as this line is redundant.
    ...
    # RA: Get requests should not be used to modify the data. Hence, removing the OR condition
    elif request.method == "POST":
        ...
        # RA: Get requests should not be used to modify the data. Hence, removing the OR condition to get data from GET
...
def use_card_view(request):
    ...
    if request.method == 'GET':
        ...
        # RA: user-input sanitization. Since it is a name, it should not have any special chars
        ...
        # RA: Parameterized the SQL queries to differentiate between the actual query and user input
        # card_query = Card.objects.raw('select id from LegacySite_card where data LIKE \'%%%s%%\'' % signature)
        # RA: For part2, we cannot query the encrypted data like this. So, we don't need this.
        # card_query = Card.objects.raw('select id from LegacySite_card where data LIKE %s', [signature])
        card_query = []
        ...
        card_id = None
        # Fetching all the decrypted cards from the DB
        for card in Card.objects.all():
            # RA: Handling the scenario if the card is already used
            if not card.used:
                card_data_json_str = card.data.decode("utf-8")
                # print(card_data_json_str)
